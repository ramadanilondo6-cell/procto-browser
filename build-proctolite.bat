...
dotnet publish "%~dp0ProctoLite\ProctoLite.csproj" -c Release -r %TARGET_RID% --self-contained true -p:PublishSingleFile=%PUBLISH_SINGLE% -p:IncludeNativeLibrariesForSelfExtract=true -p:IncludeAllContentForSelfExtract=true -o "%OUT_DIR%" >> "%LOG_FILE%" 2>&1
